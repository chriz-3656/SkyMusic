"""
Custom Application Emojis for SkyMusic
All emojis are custom Discord application emojis, not Unicode.
"""

def E(name: str, emoji_id: int) -> str:
    """Format a custom emoji for use in messages."""
    return f"<:{name}:{emoji_id}>"


# Core Controls
PLAY = E("play", 1494517456844782842)
PAUSE = E("pause", 1494517412678708292)
STOP = E("stop", 1494517441357942895)
SKIP = E("skip", 1494517437805383402)
PREV = E("prev", 1494517418633072750)

# Volume
VOL_UP = E("vol_up", 1494517406184961176)
VOL_DOWN = E("vol_down", 1494517409547423795)
VOL_MAX = E("vol_max", 1494517413843042485)
MUTE = E("mute", 1494517410898825314)

# Playback
LOOP_ALL = E("loop_all", 1494517398955900994)
LOOP_ONE = E("loop_one", 1494517398357934160)
LOOP_OFF = E("loop_off", 1494517394289332254)
AUTOPLAY = E("autoplay", 1494517361020239962)
SHUFFLE = E("shuffle", 1494517342904095554)

# Queue
QUEUE = E("queue", 1494517422554615879)
ADD = E("add", 1494517353916325888)
REMOVE = E("remove", 1494517428149813298)
CLEAR = E("clear", 1494517364065304726)
MOVE = E("move", 1494517401583353997)

# UI
SEARCH = E("search", 1494517431262253066)
SUGGEST = E("suggest", 1494517447131854577)
SUCCESS = E("success", 1494517444234969088)
ERROR = E("error", 1494517374240424076)
LOADING = E("loading", 1494517387574116493)

# Metadata
MUSIC = E("music", 1494517404788063338)
ARTIST = E("artist", 1494517503064873191)
ALBUM = E("album", 1494517599034278021)
TIME = E("time", 1494517451088597183)
LIVE = E("live", 1494517384810201219)

# Extra
FAV = E("fav", 1494517376736301208)
LIBRARY = E("library", 1494517380896915619)
DOWNLOAD = E("download", 1494517367366090916)
RADIO = E("radio", 1494517425532833954)
EQ = E("eq", 1494517370977521825)

# Status indicators
CONNECTED = SUCCESS
DISCONNECTED = ERROR
PLAYING = PLAY
PAUSED = PAUSE

# Additional UI Elements
WARNING = E("warning", 1494517451788619796)
INFO = E("info", 1494517380551159821)
DEBUG = E("debug", 1494517368769888302)
CHECK = E("check", 1494517444234969088)  # Alias for SUCCESS
BRAND = E("brand", 1494517404788063338)  # SkyMusic brand emoji (same as MUSIC)
